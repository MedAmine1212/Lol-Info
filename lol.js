...
function CheckChamps(tocheck,dont)
{
for(let i=0;i<tocheck.length;i++)
{
	champNamebyid(tocheck[i].championId,dont);
	j++;
...
var ChampsBysum = new XMLHttpRequest();
  ChampsBysum.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
		...
		dont=0;
		loadALLCHAMPS(Allchampsbysum,dont);
...
var inGame = new XMLHttpRequest();
  inGame.onreadystatechange = function() {
	  if(this.readyState == 4 && this.status==404)
	  {
		...
		{
		document.getElementById("Name"+(i+1)).innerHTML=tocheck[i].summonerName;
...
function getplayerbyname(called)
{
